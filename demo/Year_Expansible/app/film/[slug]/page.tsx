...
function getExternalAwardContext(awardKey: string, categorySlug: string, year: number): ExternalAwardContext | null {
  const key = `${awardKey}-${categorySlug}-${year}`;
...
function AwardBadgeWithPopover({ 
  ...
  const context = getExternalAwardContext(award, categorySlug, year);
...
function AwardEntryRow({ entry, filmTitle }: { entry: FilmAwardEntry; filmTitle: string }) {
  ...
  return (
    <div className="flex items-start gap-3 py-2 border-b border-border/30 last:border-0">
      ...
      <div className="flex-shrink-0 mt-0.5">
        {entry.isWinner ? (
          <div className="w-5 h-6 clip-hexagon bg-accent flex items-center justify-center" title="Rich Pick Winner">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center flex-wrap gap-x-1 mt-0.5">
          ...
          <OtherAwardsBadges 
            awards={entry.otherAwards} 
            categorySlug={entry.categorySlug}
            year={entry.year}
            currentFilm={filmTitle}
          />
        </div>
        {/* Show Rich Pick winner if this entry lost */}
        {!entry.isWinner && entry.richPickWinner && (
          <div className="text-[10px] text-muted-foreground mt-0.5">
            Rich Pick: {entry.richPickWinner}
          </div>
        )}
      </div>
...
function GroupSection({ groupId, entries, index, filmTitle }: { groupId: string; entries: FilmAwardEntry[]; index: number; filmTitle: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden px-3">
        {entries.map((entry, i) => (
          <AwardEntryRow key={i} entry={entry} filmTitle={filmTitle} />
        ))}
      </div>
    </motion.div>
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_200px] gap-8">
          ...
          <div className="space-y-2">
            ...
            {orderedGroups.map((groupId, i) => (
              <GroupSection key={groupId} groupId={groupId} entries={grouped[groupId]} index={i} filmTitle={film.title} />
            ))}
          </div>
